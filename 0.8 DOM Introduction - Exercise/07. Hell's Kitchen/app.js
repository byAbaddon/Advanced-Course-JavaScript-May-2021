function solve() {                             //  WTF  FUCKING JUDGE  ???????
  let textArr = JSON.parse(document.querySelector('#inputs textarea').value)
   let restaurants = document.querySelector('#bestRestaurant > p')
   ...
   document.querySelector('#btnSend').addEventListener('click', function () {
      for (const row of textArr) {
         const [restaurant, workerList] = row.split(' - ')
         let obj = {}
         Array.from(workerList.split(', ')).forEach((el) => {
            [key, val] = el.split(' ')
            obj[key] = +val
         })

         var sortObj = Object.entries(obj).sort((a, b) => b[1] - a[1])
         let currentAverageSalary = (Object.values(obj).reduce((a, b) => a + b) / Object.values(obj).length).toFixed(2)
         let currentBestSalary = sortObj[0][1].toFixed(2)

         if (currentAverageSalary > bestAverageSalary) {
            restaurants.textContent = `Name: ${restaurant} Average Salary: ${currentAverageSalary} Best Salary: ${currentBestSalary}`
            var collection = ''

            for (const [name, salary] of sortObj) {
               collection += `Name: ${name} With Salary: ${salary} `
            }

            workers.textContent = collection
         }
      }
   })
}
